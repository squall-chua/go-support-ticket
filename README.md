...
Ensure your MongoDB and Kafka servers are running locally. You can use the provided `docker-compose.yaml` to boot up the required infrastructure:
...
*Depending on the configuration in the Approval Config, this will either return an in-progress action, or a `Status=PENDING_APPROVAL` signaling that the action was sent for approval.*

### 3. Cancelling a Pending Action

**REST:**

```bash
curl -X POST http://localhost:8080/api/v1/action-executions/exec_123:cancel \
  -H "Authorization: Bearer $JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "reason": "Corrective action was taken manually."
  }'
```

### 4. Fetching the Audit Trail
...
External services or workers handle actions by interacting with the Kafka-backed event stream.

### 1. Listen for Trigger Events

Your worker should subscribe to the `action.execution.triggered` event type.

**Example Event Data:**

```json
{
  "event_type": "action.execution.triggered",
...
After completing the task, publish an `action.execution.executed` event back to the broker.

**Payload to Publish:**

```json
{
  "event_type": "action.execution.executed",
  "resource_id": "exec_abc123",
  "data": {
    "status": "ACTION_EXECUTION_STATUS_COMPLETED",
    "results": {
...
The **Action Service** will consume this event to mark the execution as `COMPLETED` and update the final audit trail.
